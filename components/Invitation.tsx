...
import { celebrations, couple, invitation, USING_PLACEHOLDERS } from '@/lib/content'

/**
 * The invitation proper — and now also the last page of the film rather than
 * the whole of the second half.
 *
 * After seven acts the information should feel like a calm exhale: plain
 * vertical scroll, generous space, no camera, no effects. The gold field behind
 * it is still the same canvas, so the material never changes, only the pace.
 *
 * It ends on the folio deliberately. The gatefold card is the hinge between the
 * two halves — the doors open, the canvas goes dark, and colour floods out into
 * components/Utsav.tsx. Everything that used to live below this (travel, the
 * reply, the closing mark) moved over there, where it is on colour and where
 * people will actually be looking for it.
 */
...
export default function Invitation() {
  return (
    <section data-invite className="relative z-10 px-6 pb-16 pt-[22vh]">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <div data-reveal>
          <Folio
            ...
            frontKey="vivah"
            items={celebrations.map((e) => ({
              id: e.key,
              label: e.name,
              content: (
                <div className="px-5 py-6 text-void">
                  <div className="flex items-baseline justify-between gap-3">
                    <h3 className="text-[1.35rem] font-light leading-none text-[#2a1806]">
                      {e.name}
                    </h3>
                  ...
                  {e.dress && <p className="mt-4 text-[0.8rem] italic text-[#6b4a16]">{e.dress}</p>}
